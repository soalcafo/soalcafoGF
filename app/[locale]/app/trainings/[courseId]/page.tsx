...
export default async function HrCourseDetailPage({
  params,
}: {
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-1 text-sm text-muted-foreground">
          {course.supplier?.name ? `${course.supplier.name} · ` : ""}
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button asChild>
          <Link href={`/app/trainings/${course.id}/actions/new`}>{t("action.new")}</Link>
        </Button>
      </div>
      ...
      {course.sessions.length === 0 ? (
        ...
        <div className="space-y-3">
          {course.sessions.map((s) => {
            ...
            return (
              <Link key={s.id} href={`/app/trainings/${course.id}/actions/${s.id}`} className="block">
