...
export default async function ActionDetailPage({
  ...
  const ctx = await requireAuth({ capability: "supplier.session.manage" });
  const t = await getTranslations("supplier");
  if (!ctx.tenantId || !ctx.supplierId) notFound();

  const action = await forSupplier(ctx.tenantId, ctx.supplierId, (tx) =>
  ...
  async function addModule(formData: FormData) {
    "use server";
    const c = await requireAuth({ capability: "supplier.session.manage" });
    if (!c.tenantId || !c.supplierId) throw new Error("No supplier context");
    const name = String(formData.get("name") ?? "").trim();
    const durationHours = Number(formData.get("durationHours") ?? 0);
    const contents = String(formData.get("contents") ?? "").trim() || null;
    if (name) {
      await forSupplier(c.tenantId, c.supplierId, (tx) =>
    ...
    redirect(`/${locale}/portal/trainings/${courseId}/actions/${actionId}`);
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href={`/portal/trainings/${courseId}`} className="text-sm text-muted-foreground hover:text-foreground">
